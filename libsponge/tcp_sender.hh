...
class Timer {
  ...
  public:
    ...
    bool check_timeout() const {
      return _is_running && _time_count >= _timeout;
    }
...
class TCPSender {
  private:
    ...
    // retransmisssion timer
    Timer _timer;

    // sent but not acked, outstanding_segment
    std::queue<std::pair<uint64_t, TCPSegment>> _outstanding_segment{};
